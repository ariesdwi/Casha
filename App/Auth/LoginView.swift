        ...
        if #available(iOS 16.4, *) {
            NavigationStack {
                ZStack {
                    ...
                    // MARK: - Toast (Error or Success)
                    if let message = state.toastMessage {
                        VStack {
                            Spacer() // push toast to bottom
...
extension LoginView {
    ...
    private var signUpView: some View {
        HStack {
            ...
            if #available(iOS 16.0, *) {
                Button("Sign Up") { showRegisterSheet = true }
                    .foregroundColor(.cashaPrimary)
                    .fontWeight(.semibold)
            }
        }
    ...
    private func performLogin() async {
        isLoading = true
        hideKeyboard()
        await state.login() // Updates toastMessage for success or error
        isLoading = false
    }
    ...
    private func handleAppleSignIn(result: Result<ASAuthorization, Error>) {
        ...
        case .success(let authResults):
            state.toastMessage = "Apple login successful ✅"
